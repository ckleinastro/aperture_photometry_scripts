...
progenitor_image_name = "h_long_PTF2.2.51_coadd.fits"
# region_file = "SN.266.3.reg"
# region_file = progenitor_image_name.split("_")[2] + ".reg"
region_file = "target.reg"

sextractor_bin = "/usr/local/bin/sex"
weight_image_name = progenitor_image_name.replace("coadd", "coadd.weight")
# weight_image_name = "h_long_RATS.40.146_coadd.weight.convertedwcs.fits"
...
def convert_header_floats(image_filename):
    ...
    system("rm " + input_image)
    system("mv " + input_image.replace(".fits", ".convertedwcs.fits") + " " + input_image)
...
# Run function to make sure that the headers in the images have float vals for
# the WCS information (needed by Source Extractor)
# convert_header_floats(progenitor_image_name)
# convert_header_floats(weight_image_name)
...
for g in range(order):
    for h in range(order):
        ...
        output_hdu.header.update("IMAGEW", 512)
        output_hdulist = pyfits.HDUList([output_hdu])
        ...
        # apertures.
        sexcat_fake_stars = make_sex_cat_fake_stars(
            progenitor_image_name.replace("coadd", "fakestars1_" + str(h) + "_" + str(g)), 
            progenitor_image_name, weight_image_name, aperture_size)
            
        sexcat_fake_stars_no_weight = make_sex_cat_fake_stars_no_weight(
            progenitor_image_name.replace("coadd", "fakestars1_" + str(h) + "_" + str(g)), 
            weight_image_name, aperture_size)
